...
public class DropScheduler {
    ...
    public void activate() {
        int delay = plugin.getSettings().getSpawnCooldown();
        int tickDelay = delay * 20;
        plugin.getHologramSystem().updateTimer(delay);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                plugin.getHologramSystem().updateTimer(remaining);
    ...
    private void spawn() {
        ...
        List<Player> nearby = plugin.getPayoutSystem().fetchNearby();
    ...
    private List<ItemStack> fetchPool() {
        ...
        int qty = plugin.getSettings().getInternalAmount();

        for (int i = 0; i < qty; i++) {
            ItemStack stack = plugin.getLootPool().random();
