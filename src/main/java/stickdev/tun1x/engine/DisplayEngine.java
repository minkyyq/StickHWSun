...
public class DisplayEngine {
    ...
    public void init() {
        ...
        double height = core.cfg().getVerticalShift();
    ...
    private String renderHeader() {
        return TextUtil.color(core.cfg().getHeaderText());
    }

    private String renderMode() {
        RewardCycle mode = core.cycleEngine().getActive();
        String colored = TextUtil.color(mode.getDisplay());
        return TextUtil.color(core.cfg().getStatusText().replace("%cycle%", colored));
    }

    private String renderCountdown() {
        return TextUtil.color(core.cfg().getCountdownText().replace("%time%", String.valueOf(countdown)));
    }
}
